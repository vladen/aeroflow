...
the context data (see [run](#Flow+run) method for additional information about context data).
...
```
<a name="Flow+catch
Returns new flow suppressing error, emitted by this flow, or replacing it with alternative data source."></a>
## Flow#catch
Returns new flow suppressing error, emitted by this flow, or replacing it with alternative data source.([alternative]) ⇒ <code>[Flow](#Flow)</code>
**Kind**: global function  
**Params**
...
and context data (see [run](#Flow+run) method for additional information about context data).
The result, returned by this function, as well as any other value passed as alternative,
is adapted with suitable adapter from [aeroflow.adapters](aeroflow.adapters) registry and emitted to this flow.
