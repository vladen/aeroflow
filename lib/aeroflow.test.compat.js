...
describe('aeroflow', function () {
  ...
  describe('concat', function () {
    it('is static method', function () {
      return assert.isFunction(aeroflow.concat);
    });
    ...
    it('concatenates several values into one flow', function (done) {
      ...
      (_aeroflow = aeroflow).concat.apply(_aeroflow, [1, 2, 3]).run(onNext, onDone);
    });
    it('concatenates several arrays into one flow', function (done) {
      ...
      (_aeroflow2 = aeroflow).concat.apply(_aeroflow2, _toConsumableArray(Array(times).fill(values))).run(onNext, onDone);
    });
  });
