...
describe('aeroflow', function () {
  ...
  describe('append', function () {
    ...
    it('append several values to the flow', function (done) {
      var count = 0,
          values = [1, 2, 3],
          onDone = function onDone() {
        assert.strictEqual(count, values.length);
    ...
    it('append several arrays to the flow', function (done) {
      var _aeroflow;
      ...
      (_aeroflow = aeroflow(values)).append.apply(_aeroflow, _toConsumableArray(Array(times - 1).fill(values))).run(onNext, onDone);
    });
  });
  ...
  describe('repeat', function () {
    ...
    it('creates flow emitting undefined values', function (done) {
      return aeroflow.repeat().take(1).run(function (next) {
