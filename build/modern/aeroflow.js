(function (global, factory) {
  ...
  const CLASS_PROMISE = 'Promise';
  const CLASS_REG_EXP = 'RegExp';
  ...
  const isDate = classIs(CLASS_DATE);
  const isError = classIs(CLASS_ERROR);
  ...
  const isPromise = classIs(CLASS_PROMISE);
  const isRegExp = classIs(CLASS_REG_EXP);
  ...
  const throwError = error => { throw isError(error) ? error : new Error(error); };
  ...
  function toSet(transformer) {
    return new Aeroflow(arguments.length === 0
      ? toSetEmitter(this[SYMBOL_EMITTER])
    ...
    return new Aeroflow(arguments.length
      ? isFunction(transformer)
        ...
        : toArrayTransformingEmitter(
            this[SYMBOL_EMITTER],
            constant(transformer))
      : toArrayEmitter(this[SYMBOL_EMITTER]));
  }
  ...
  */
  function timestamp() {
    return new Aeroflow(timestampEmitter(this[SYMBOL_EMITTER]));
  }
  ...
  function tap(callback) {
    return isFunction(callback)
      ? new Aeroflow(tapEmitter(this[SYMBOL_EMITTER], callback))
      : this;
  }
